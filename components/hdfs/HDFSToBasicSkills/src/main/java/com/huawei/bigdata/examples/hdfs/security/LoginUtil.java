...
import com.huawei.bigdata.examples.hdfs.java_api.ADemoForDoc;
...
public class LoginUtil {
                ...
                throws IOException
        {
            ...
            String krb5ConfPath = ADemoForDoc.class.getClassLoader().getResource("conf/krb5.conf").getPath();
            String userKeytabPath = ADemoForDoc.class.getClassLoader().getResource("conf/user.keytab").getPath();
            Properties properties = new Properties();
            FileInputStream inputStream = new FileInputStream(userKeytabPath);
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream));
            String str = null;
            r:
            while((str = bufferedReader.readLine()) != null)
            {
                if("HADOOP".equalsIgnoreCase(str.substring(0,6))){
                    int a = str.indexOf("M");
                    int b = str.indexOf("\\");
                    userPrincipal = str.substring(a+1,b-1).trim();
                    break r;
                }
            }
            inputStream.close();
