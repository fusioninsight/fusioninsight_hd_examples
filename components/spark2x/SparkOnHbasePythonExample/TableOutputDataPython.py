...
# 创建SparkSession并设置kryo序列化
spark = SparkSession\
        .builder\
        .appName("TableOutputData")\
        .config("spark.serializer","org.apache.spark.serializer.KryoSerializer")\
        .config("spark.kryo.registrator", "com.huawei.bigdata.spark.examples.MyRegistrator")\
        .getOrCreate()

# 向sc._jvm中导入要运行的类
java_import(spark._jvm, 'com.huawei.bigdata.spark.examples.TableOutputDataPython')

# 创建类实例并调用方法，传递sc._jsc参数
spark._jvm.TableOutputDataPython().readtable(spark._jsc)
